...
???+ warning
    Ensure you can ping your hostname before proceeding as otherwise the initialisation will fail. If not, edit your `/etc/hosts` file and add an entry for `127.0.1.1  donaldson.local`.
...
```bash
kubectl label node banks.local topology.kubernetes.io/region=house; \
kubectl label node banks.local topology.kubernetes.io/zone=mancave; \
kubectl label node banks.local topology.rook.io/rack=dellboy
```
