...
???+ warning
    Ensure you can ping your hostname before proceeding as otherwise the initialisation will fail. If not you can either:
    
    * edit your `/etc/hosts` file and add an entry for `127.0.1.1  donaldson.local`; or
    * use `hostnamectl` along the lines of `sudo hostnamectl set-hostname donaldson.local`;
...
```bash
kubectl label node banks.local \
