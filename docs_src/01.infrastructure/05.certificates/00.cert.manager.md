...
As per responses to issue [#456](https://github.com/kubernetes-sigs/external-dns/issues/456#issuecomment-404229331) we need to pass in the highlighted flags such that our ingress rules are exposed later on correctly to external-dns. See [configuration](https://github.com/helm/charts/tree/master/stable/nginx-ingress#configuration) for details.
```bash hl_lines="7 8"
helm install my-ingress stable/nginx-ingress \
    ...
    # --set controller.service.externalTrafficPolicy=Local  # this will give an IP of 10.100.80.140 etc.
```
