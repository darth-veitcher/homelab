...
# Set Secrets
As with OpenLDAP we will store our sensitive credentials into a secret.
...
??? example "Identity Provider Configuration (Settings)"
    ![Page 1](img/Screenshot 2019-12-23 at 16.39.45.png)
    ![Page 2](img/Screenshot 2019-12-23 at 16.39.57.png)
    ![Page 3](img/Screenshot 2019-12-23 at 16.40.08.png)
...
<!-- # Create new User Mapper
so that we can use `posixGroups` properly we need to be able to map the `uid` of our user through.

Create new new mapper with the following details.
...
* `LDAP Groups DN`: ou=UnixGroups,ou=Groups,dc=jamesveitch,dc=dev
...
In order to access the equivalent settings for your user in the `Development` realm click on the `Client Roles` drop-down and then select `realm-management` then assign `realm-admin`.

??? example "Group Role Mappings"
    ![Group Mapping](img/Screenshot 2019-12-21 at 16.18.18.png)
    ![Realm Admin](img/Screenshot 2019-12-23 at 17.01.32.png)

Navigate to [https://auth.jamesveitch.dev/auth/realms/Development/account/](https://auth.jamesveitch.dev/auth/realms/Development) and you should be able to login now as that user.
