...
sed -i.bak 's/# encryptedDevice:/encryptedDevice: true/' cluster.yaml
```
...
In this example, port `30995` will be opened to expose port `8443` from the `ceph-mgr pod`. Find the ip address of the node.
...
???+ tip "Finding the IP address"
    ...
    Now you can enter the URL in your browser as https://192.168.0.95:30995 and the dashboard will appear.
